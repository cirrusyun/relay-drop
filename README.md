...
新加入笔记的图片会在 VPS 内自动执行一次简体中文和英文 OCR，识别出的文字追加到当前正文并保存。OCR 会先将图片缩放、灰度增强，限制为单任务运行，并设置处理超时；超过 20 MiB 的图片不执行 OCR，但仍会正常保存和显示。图片工具栏的 `OCR` 可以手动重试。点击 `PDF` 会打开系统打印窗口，只排版当前笔记的标题、可复制文字和展开图片；选择“存储为 PDF”或“另存为 PDF”即可保存。
