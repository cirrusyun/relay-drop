...
- 轻量记事本：顶部小入口打开独立页面，剪贴板一键存为笔记，也可新建、编辑、复制和删除笔记；支持选择或直接粘贴图片，笔记默认永久保存。
- 直接粘贴截图：图片会自动上传到文件区。
- 多文件拖拽上传与逐文件进度。
...
在笔记正文中粘贴截图，或点击“图片”选择 PNG、JPEG、GIF、WebP、AVIF，图片会进入同一个文件中转区，并立即保存到当前笔记；直接刷新也不会丢失。笔记只保存文件标识并显示服务器生成的小缩略图，不会再复制一份原图。点击缩略图才会按需读取原图并放大预览，可在适应窗口和原始尺寸之间切换，平时不会反复加载大图。把图片从笔记中移除不会删除中转区文件；如果从中转区删除原文件，笔记中会显示图片已删除。图片上传期间会暂时阻止离开当前笔记，避免上传结果落到错误的笔记里。
